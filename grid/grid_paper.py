...
spring_radius = 2
spring_stiffness_constant = 1000000000000
...
def detect_circle_collision(p1, p2):
    ...
    if dist_norm <= ((particle_radius * 2) / height):
        ...
        # constraint so that particles don't overlap
        overlap_dist = ((particle_radius * 2) / height) - dist_norm
        move_dist = overlap_dist / 2 * dist_norm_vec +0.00000001
        # x[p1] -= move_dist
        # x[p2] += move_dist
...
def initialize():
    ...
    mid_x = start_x - (g_cols - 1) * spring_len_x / 2
    print(mid_x)
    gf.set_positions(x, g_rows, g_cols, mid_x, start_y, spring_len_x, spring_len_y)
    # gf.set_positions(x, g_rows, g_cols, 0.5, start_y, spring_len_x, spring_len_y)
