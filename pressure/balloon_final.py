...
spring_radius = 2.5
spring_stiffness_constant = 500000000
...
def draw_bounding_box(coords):
    for i in range(len(coords)+1):
        gui.line(coords[(i)%len(coords)], coords[(i+1)%len(coords)], radius=spring_radius/3, color=0xFFFFFF)
...
def draw_triangulation():
    ...
    for i in ti.static(range(num_particles)):
        gui.line( center, x[i], radius=spring_radius/3, color=0xFFFFFF)
